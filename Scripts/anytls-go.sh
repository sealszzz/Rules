...
ANYTLS_STATE_DIR="/var/lib/anytls-go"
ANYTLS_CONF_DIR="/etc/anytls-go"
ANYTLS_CONF_FILE="${ANYTLS_CONF_DIR}/config.json"
ANYTLS_BIN="/usr/local/bin/anytls-go"
ANYTLS_SERVICE_NAME="anytls-go"
...
install_anytls_release() {
  ...
  [ -n "$ANYTLS_TAG" ] || { echo "FATAL: cannot detect anytls-go tag" >&2; exit 1; }

  case "$(uname -m)" in
    x86_64|amd64)  ASSET="anytls-go-linux-amd64-${ANYTLS_TAG}.tar.gz" ;;
    aarch64|arm64) ASSET="anytls-go-linux-arm64-${ANYTLS_TAG}.tar.gz" ;;
  ...
  bin="$(find "$tmpd/unpack" -type f -name anytls-go -perm -u+x | head -n1 || true)"
  [ -n "$bin" ] || { echo "FATAL: anytls-go binary not found" >&2; exit 1; }
...
BIN_VER="$("$ANYTLS_BIN" --version 2>/dev/null || "$ANYTLS_BIN" version 2>/dev/null || true)"
...
echo "anytls-go tag: ${ANYTLS_TAG:-unknown}"
echo "anytls-go bin: ${BIN_VER:-unknown}"
