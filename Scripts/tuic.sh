...
port_used_udp() {
  local p="$1"
  ss -Hulnp 2>/dev/null | awk -v P=":$p" '$5 ~ P {print; exit 0; } END{exit 1}'
}
if port_used_udp "$TUIC_PORT"; then
  echo "提示：UDP/$TUIC_PORT 已被占用（例如 ShadowQUIC）。"
  if [ "${TUIC_PORT}" = "443" ]; then
    echo "自动将默认端口切换为 4443（可通过导出 TUIC_PORT 覆盖）。"
    TUIC_PORT=4443
  fi
fi
...
if [ "$USE_CARGO" -eq 1 ]; then
  ...
  install_release_tuic() {
    ...
    curl -fL -o "$tmpd/tuic-server" "https://github.com/Itsusinn/tuic/releases/latest/download/${asset}"
