...
if [ "$need_install" -eq 1 ]; then
  ...
  asset="$(echo "$assets" | grep -i -E "${pat_arch}.*(linux|unknown-linux).*gnu.*(\.tar\.gz|\.tar\.xz|\.zip)$" | head -n1 || true)"
  [ -n "$asset" ] || asset="$(echo "$assets" | grep -i -E "${pat_arch}.*(linux|unknown-linux).*musl.*(\.tar\.gz|\.tar\.xz|\.zip)$" | head -n1 || true)"
