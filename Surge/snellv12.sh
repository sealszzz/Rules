...
# 全局变量：最终选定的 Snell 版本（形如 v5.0.0b3 或 v6.0.0.0）
SNELL_VERSION=""
...
get_latest_snell_version() {
  ...
  # 抓取页面
  local html
  html=$(curl -fsSL --connect-timeout 5 -m 10 "$url")

  # ---- 优先 beta：匹配 x.y.zbN ----
  # 提取：snell-server-vX.Y.ZbN
  local v_beta
...
# 获取 Snell 下载 URL（根据架构自动拼接）
get_snell_download_url() {
  local version="$1"   # 形如 v5.0.0b3 或 v6.0.0.0
  local arch
  arch=$(uname -m)
...
# url="$(get_snell_download_url "$SNELL_VERSION")"
# echo "将安装版本: $SNELL_VERSION"
# echo "下载地址: $url"
...
get_user_port() {
    while true; do
        ...
        if [[ -z "$PORT" ]]; then
            PORT=$(shuf -i 30000-39999 -n 1)  # 生成 30000-39999 随机端口
